...
- Two distinct current feature prototypes exist: full-height duel field + card-list dialog — sources: `artifacts/PROTO_2026_08_12_full_height_field.html`, `artifacts/PROTOTYPE_card-list-dialog.html`.
- Card-list feature itself has 2 HTML variants: working evaluator + fixed approved state — sources: `artifacts/PROTOTYPE_SPEC_card-list-dialog.md`, `docs/feature/PROTOTYPE_card-list-dialog.html`.
- Full-height prototype is validated, ready to implement; prod still uses fixed `1280×720` geometry + `DuelHeaderBar` — sources: `artifacts/DESIGN_2026_08_13_full_height_duel_field.md`, `src/field/duel-field-layout.ts`, `src/app/App.svelte`.
- Card-list prod impl exists but predates approved redesign — sources: `src/app/components/duel-field/ZoneListDialog.svelte`, `artifacts/PROTOTYPE_SPEC_card-list-dialog.md`.
- `.pi/artifacts/*prototype*.html` files are untracked local mirrors, not authority — source: `git status --short`.
- Approved card-list spec has no unresolved variable — source: `artifacts/PROTOTYPE_SPEC_card-list-dialog.md:121`.
- Full-height design leaves exactly 3 product choices open — source: `artifacts/DESIGN_2026_08_13_full_height_duel_field.md:505-509`.
...
- Card-list authority: `artifacts/PROTOTYPE_SPEC_card-list-dialog.md` + approved fixed prototype own frozen visuals/interactions, subject to Worker/privacy/a11y + field-local `FloatingFieldWindow` bounds.
