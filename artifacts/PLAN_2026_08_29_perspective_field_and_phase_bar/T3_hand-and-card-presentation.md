...
- Hand bands (both carriers): counter-tilt `rotateX(-20deg)` so cards face the camera (must equal `FIELD_TILT_DEG` negated — derive from the constant, not a second literal). The non-scrolling carrier owns this 3D transform; its scrolling viewport and cards stay local 2D for fan, droop, focus, and drag.
...
```svelte
<!-- FieldBoard passes inset geometry.cardHeight into each non-scrolling HandBand carrier. -->
<!-- HandBand.svelte: per-card 2D CSS custom props, passed via CardControl style -->
...
.duel-field-hand-band {
  transform: translate(-50%, -50%) translateZ(1px)
    rotateX(var(--hand-upright, 0deg));
}
.duel-field-card.is-hand-item {
  transform: rotate(var(--card-fan, 0deg)) translateY(var(--card-droop, 0px));
}
.duel-field-card.is-identity-known.is-hand-item:not(.is-pinned):focus-within {
  transform: scale(1.35) rotate(var(--card-fan, 0deg))
    translateY(var(--card-droop, 0px));
}
.duel-field-board__plane { --hand-upright: -20deg; }  /* FieldBoard derives from FIELD_TILT_DEG; carrier consumes it */
```
...
- **Integration links:** trigger `HandBand` render → observe in Chromium: `getComputedStyle(handCarrier).transform` is a `matrix3d` (rotateX present), while scrolling viewport cards and field cards retain 2-D `matrix` transforms.

## TDD

1. **Red** — HandBand component test: 5 cards → middle card `--card-fan: 0deg`, outermost `±5deg`, exact droop `[2.7520000000000002, 1.3760000000000001, 0, 1.3760000000000001, 2.7520000000000002]` px from inset `cardHeight`; CardControl field layout ignores fan.
2. **Green** — props + CSS.
...
| droop | HandBand, 5 cards, inset `cardHeight` | exact `[2.7520000000000002, 1.3760000000000001, 0, 1.3760000000000001, 2.7520000000000002]` px values |
...
| e2e visual | Chromium, started duel | non-scrolling hand carrier computed transform is matrix3d; scrolling hand card remains 2-D matrix; screenshot artifact for checklist |
...
- [x] manual: hands fan and stand up, both sides; defense-position card still reads rotated; no card touches a zone border — Verify: focused Chromium run passed carrier `matrix3d` + screenshot evidence, hand cards retained 2-D transforms; component suite passed opponent/defense orientation and inset geometry assertions.
...
- [x] R7. Derive hand droop from explicit inset `geometry.cardHeight` passed by FieldBoard into HandBand; pin exact five-card droop values. Verify: `npx vitest run tests/component/HandBand.test.ts tests/component/FieldBoard.test.ts` passed 2 files, 14 tests with exact `[2.7520000000000002, 1.3760000000000001, 0, 1.3760000000000001, 2.7520000000000002]` px values.
- [x] R8. Align interface, rationale, integration link, and test plan with final carrier architecture: non-scrolling hand carrier owns `rotateX(-20deg)` in preserved 3D; scrolling viewport/cards retain local 2D fan/droop/focus. Verify: interface/test-plan text names carrier `matrix3d` and card 2-D `matrix`, with no card-level `rotateX` claim; `git diff --check` passed.
- [x] R9. Run final focused tests, headless checks, Chromium proof, build, and residue/secret scans; create new corrective commit. Verify: focused Vitest passed 2 files/14 tests; `npm run check:headless` passed (23 legacy, 157 files/1788 unit, 15 files/39 integration, vendor/assets/snapshot verification); focused Chromium proof passed 2/2; full `e2e/duel-smoke.spec.ts` passed 40/41 with 1 seed-dependent skip; `npm run build` passed (426 modules, all domain budgets); `git diff --check`, source/test secret scan, and residue scan passed with 0 hits; new commit is `fix(duel): derive hand droop from inset card height` with no staged files.
