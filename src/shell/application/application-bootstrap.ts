...
export async function bootstrapApplication(
  ...
  try {
    ...
    const store = await openProgressiveContentStore(null);
    service = createApplicationService({
      ...
      isHome: () =>
        globalThis.location.hash === "#/" || globalThis.location.hash === "",
    });
    try {
      ...
      await session.close();
      return { bootstrap: null, gate, application: service.application };
    } catch (error) {
      if (!(error instanceof Error) || error.message !== "APP_CONTENT_REQUIRED")
        throw error;
    }
    // T10 owns explicit network discovery through application-service, after local readiness.
    // An empty selector never opens the retired installer or grants legacy readiness.
    return {
      bootstrap: null,
