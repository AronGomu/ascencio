# T15: Budgets + checklist + glossary + docs

**Plan:** `./artifacts/PLAN_2026_08_27_deck_selection_screen.md`
**Depends:** T11, T12, T14
**Commit outcome:** Full gate green — `npm run check:headless`, `npm run build:verify` (budgets re-measured where the new shared chunk moved bytes), full e2e; `artifacts/manual_test_checklist.md`, `docs/GLOSSARY.md`, `docs/architecture/architecture.md` and the design doc's status current. Round closable.
...
- Assumptions in force: T10-T14 landed; `src/deck-select/` is imported by shell, deck-editor and story — its bytes count into each domain closure measured by `scripts/verify-browser-build.ts`; budget thresholds are code, raised only deliberately with the measurement quoted (repo precedent: the T11/T21 comments already in that file).
...
- Gates, in order, each output captured:
  ...
  4. `npm run test:e2e` — full Playwright suite, desktop + any narrow-viewport specs added in T10/T14.
...
- Residual risks to log in the final report: colour-only semantics (no contrast/colour-blind pass — pre-existing flagged debt now extended to this screen); story kebab management deferred (T14 assumption); graphify staleness if skipped.
...
- **From Depends:** T10-T14 commits on the branch; every suite already green per-ticket — this ticket proves them green together.
...
- [ ] 5. Update `artifacts/manual_test_checklist.md` per Requirements.
...
- [ ] app functional end to end: free play, library, story all on the new screen
