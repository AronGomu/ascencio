...
- Decision (ADR-031 `docs/ADR/031_ADR_halo_color_semantics_v2.md`, confirmed by grill — `artifacts/GRILL_2026_08_16_duel_feedback_round_4/ANSWERS.md` round 1 Q10): list dialog aligns to field: actionable entry hover/focus = green ring; selected = orange (persisted class, unchanged); unavailable (over-maximum or hard-locked) = red ring; neutral (zero choices) = no ring ever. PLUS field cards: while a card-targeting prompt is active, hovering ANY non-candidate field card shows a red ring — hover-only, never persistent, green/orange field rules untouched.
