...
import { STORY_SCREENS, type StoryState } from "../model/story-state.ts";

export const STORY_STORAGE_KEY = "ygo.story.v1";
export type StorySaveSlot = "manual" | "autosave";
export type SaveResult =
  { readonly ok: true } | { readonly ok: false; readonly message: string };
export type StorySlots = {
  readonly manual: StoryState | null;
  readonly autosave: StoryState | null;
  readonly latest: StorySaveSlot | null;
};
export type LoadSlotsResult =
  | { readonly ok: true; readonly slots: StorySlots }
  | { readonly ok: false; readonly message: string };

type StoredEnvelope = StorySlots & { readonly schemaVersion: 1 };
const EMPTY_SLOTS: StorySlots = {
  manual: null,
  autosave: null,
  latest: null,
};

export function loadStorySlots(storage?: Storage): LoadSlotsResult {
  try {
    const target = storage ?? globalThis.localStorage;
    const raw = target.getItem(STORY_STORAGE_KEY);
...
export function saveStoryState(
  state: StoryState,
  storage?: Storage,
  slot: StorySaveSlot = "manual",
): SaveResult {
  try {
    const target = storage ?? globalThis.localStorage;
    const existing = loadStorySlots(target);
    ...
    target.setItem(STORY_STORAGE_KEY, JSON.stringify(envelope));
    return { ok: true };
  } catch (error) {
    return {
      ok: false,
...
export function resetStoryStorage(
  storage?: Storage,
  slot?: StorySaveSlot,
): SaveResult {
  try {
    const target = storage ?? globalThis.localStorage;
    if (slot === undefined) {
      target.removeItem(STORY_STORAGE_KEY);
      return { ok: true };
    }
    const existing = loadStorySlots(target);
    if (!existing.ok) return existing;
    const remaining: StorySlots = {
      ...existing.slots,
      [slot]: null,
      latest:
...
function isStoredEnvelope(value: unknown): value is StoredEnvelope {
  ...
  return (
    ...
    (envelope.manual === null || isStoryState(envelope.manual)) &&
    (envelope.autosave === null || isStoryState(envelope.autosave)) &&
