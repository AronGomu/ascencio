...
These files were dirty **before** this run and must never be staged: `.gitignore`, `README.md`, `docs/README.md`, `docs/architecture/**`, `docs/developer-guide/**`, `docs/duel-field-architecture.html`, `docs/duel-field-validation-references.html`, `playwright.config.ts`, `vite.config.ts`, `test-results/**`, and untracked `.claude/`, `.pi/`, `.pi-subagents/`, `.agents/`, `.agentsystem/`, `.dev/`, `.tmp/`, `CLAUDE.md`, `AGENTS.md`, `context.md`, `.graphifyignore`, `artifacts/HANDOFF_2026_08_09_duel_field_ux_overhaul.md`. Also leave `artifacts/PLAN_2026_08_08_duel_field_ux_overhaul/T8_*.md` and `T11_*.md` alone — those are the parent's edits to tickets you do not own.
