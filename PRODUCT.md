...
- **Chosen visual direction (owner-picked, 2026-09-02): "Basilica Slate".** Dark blue-black ground (`#080d16`, kin of the shipped `#08101f` duel palette), translucent frosted blue-tinted glass panels with backdrop blur, rectangular geometry with shallow 45° chamfers, gold `#d3b268` as ceremonial primary (logo warmth, primary action, phase marker), white `#eef1f7` secondary carrying structure, serif type — Forum for display/inscriptional capitals, Source Serif 4 for body — self-hosted (offline constraint). Angelic golden-white `ASCENCIO` wordmark. Picked from four renditions of the owner-pinned world (dark + translucent glass + angular + gold/white + classy serif); prototype anchored at the commit introducing `artifacts/PROTO_2026_09_02_ascencio_brand_variations.html`. DESIGN.md is written from the built world once the restyle round ships.
