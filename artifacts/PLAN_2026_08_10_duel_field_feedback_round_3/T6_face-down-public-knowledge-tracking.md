...
- [ ] manual fixture check: turn a known face-up opponent card face-down; hover it; preview keeps art, name and effect — human step, published to `artifacts/manual_test_checklist.md` (automated proxy: component test `hovering a known face-down board card reports code while rendering back art`)
- [ ] manual privacy check: set an unrevealed opponent card from hand; hover it; preview says hidden and no card identity appears in snapshot/diagnostics — human step, published to `artifacts/manual_test_checklist.md` (automated proxy: `does not learn an unrevealed opponent set from hand`)
