...
import OutcomeScreen from "../../../src/story/screens/OutcomeScreen.svelte";
import RewardScreen from "../../../src/story/screens/RewardScreen.svelte";
...
describe("outcome and progression", () => {
  ...
  it("reveals reward once, updates reducer objective/map, and shows stable autosave", async () => {
    ...
    state = reduceStory(state, { type: "battle-result", result: "win" });
    state = reduceStory(state, { type: "continue-outcome" });
    state = reduceStory(state, { type: "acknowledge-reward" });
