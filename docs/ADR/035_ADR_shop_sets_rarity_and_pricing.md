...
2. **Offline-first delivery.** The JSON is a `public/` asset, lazy-fetched on first shop entry, validated structurally on load, cached in Cache Storage (`story-shop-data`, network-first with cache fallback — rev. 4). Every online visit revalidates and replaces the cached payload; a failed or invalid fetch serves the cache, so after one online session the shop still browses fully offline — PWA rule holds. It never enters the story JS chunk, so the domain budget measures UI, not data.
...
4. **Rarity: printed first, inferred fallback.** A card's rarity = its printed rarity from the set data; printed at several rarities in one set → the highest of its **base** printings wins; a foil re-run of a card the set already sells (Ultimate, Ghost, Parallel, Gold, Collector's…) is not a second slot, and only a card the set printed as nothing else keeps one. Codes the data misses fall back to pure ordered rules over `DeckBuilderCardView` (ATK ≥ 3000 → secret; extra-deck → ultra; ritual or ATK ≥ 2000 → super; typed spell/trap or level ≥ 5 effect → rare; else common), then `common`. Off-union printed rarities map down (Short Print → common, Parallel → base, Gold → ultra, later specials → secret). **Rarity is never saved**, so this fold is retroactive: a save holds `collection[code]` counts only and `resolveCardRarity` resolves the tier on every read, so an existing collection is repriced the moment the new data loads — 568 codes drop a tier, 541 of them off Ultimate Rare (500 DP) and 27 off Ghost Rare (1000 DP), down to rare/super/ultra/secret. Accepted rather than migrated: those prices were the bug, and a save that kept them would keep minting DP.
5. **Pricing.** `PACK_PRICE_DP = 150` (rev. 3 — must exceed a pack's expected sell value of ≈138–142 DP, or buy → open → sell mints DP without bound), `PACK_SIZE = 9` (8 commons + 1 rare-or-better, seeded-RNG-testable generator). Sell ladder common→ghost: 10 / 25 / 50 / 100 / 250 / 500 / 1000. **Singles: buyable at 4× sell price once their set is released** (common 40 DP), from the set card-list window. Start wallet 1000 DP ≈ six packs. **A pack is only buyable while its set is `released`:** `reduceStory`'s `buy-packs` takes the flag on the command, resolved from the set list by `isSetReleased` (`src/story/shop/data/shop-set-data.ts`), because the reducer is pure and never sees the fetched document.
